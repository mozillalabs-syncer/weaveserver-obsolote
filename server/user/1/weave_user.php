...
class WeaveAuthenticationMozilla implements WeaveAuthentication
{
	...
	function get_new_node_location($username)
	{
		if (defined('WEAVE_REGISTRATION_THROTTLE_DB'))
		{
			...
			try
			{
				$select_stmt = 'select node from available_nodes where ct > 0 order by actives limit 1';
			...
			try
			{
				$insert_stmt = 'update available_nodes set ct = ct - 1, actives = actives + 1 where node = ?';
				$sth = $this->_dbh->prepare($insert_stmt);
				$sth->execute(array($result));
			}
			catch( PDOException $exception )
